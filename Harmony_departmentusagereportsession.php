<?php
	...
	class PDF extends FPDF
	{
		...
		// Load admission data
		function LoadAdmData($department, $reportYear, $reportMonth)
		{
			//Connect to database
			include ('pagecomponents/connectDB.php');
			...
			FROM admissions 
			WHERE admission_date BETWEEN "'.$_SESSION['report_year'].'-0'.$_SESSION['report_month'].'-01" AND "'.$_SESSION['report_year'].'-0'.$_SESSION['report_month'].'-31" AND department_id = '.$department;
		...
		function AdmissionTable($admheader, $admdata, &$patientIdArray, $todaysDate, $departmentName, $departmentnumber, $reportYear, $reportMonth){
			...
			$admdata = $this->LoadAdmData($departmentnumber, $reportYear, $reportMonth);
        ...
        function iterateDepartments($admheader, $depdata, $patientIdArray, $todaysDate)
		{
			...
			foreach ($departments as $key=>$element) {
				...
				$this->AdmissionTable($admheader, $admdata, $patientIdArray, $todaysDate, $querydepartmentname, $querydepartmentno, $reportYear, $reportMonth);
			}
	...
	$admheader = array('Number of Admissions', 'Number of Discharges','Average length of stay (days)', 'Average Age of Patients','Number of returning patients');
	$admissionIdArray = array();
	$patientIdArray = array();
	//unset($_SESSION['report_month']);
	//unset($_SESSION['report_year']);
	$pdf->iterateDepartments($admheader, $depdata, $patientIdArray, $todaysDate, $reportYear, $reportMonth);
	$pdf->Output();
