<?php
	...
	class PDF extends FPDF
	{
		...
		function Header(){
			...
			//Title
			$this->MultiCell(88,8,'DEPARTMENT USAGE REPORT',0,'C');
		...
		function AdmissionTable($admheader, $admdata, $patdata, &$patientIdArray, $todaysDate){
    		...
    		// Header
    		$this->Cell(190,10,"DEPARTMENT ADMISSIONS STATISTICS",1,0,'C',true);
    		$this->Ln();
        ...
        //function iterateDepartments($depdata, &$departmentListing)
		//{
		//	while($row=mysqli_fetch_array($depdata))
		//	{
		//		array_push($departmentListing, $row['department_id']);
		//	}

		//	for($i=0;$i<count($departmentListing);$i++)
		//	{
		//		$admdata = $this->LoadAdmData($file,$departmentListing[i]);
		//		AdmissionTable($admheader, $admdata, $patdata, $patientIdArray, $todaysDate);
		//	}
		//}
